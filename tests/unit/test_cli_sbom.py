...
class TestFormatBytes:
    """Tests for the format_bytes helper."""
    ...
    def test_mebibytes(self):
        ...
        assert format_bytes(int(1.5 * 1024 * 1024)) == "1.5 MiB"
...
# ---------------------------------------------------------------------------
# Snapshot ID validation tests (Requirements 1.2, 1.4, 1.5, 5.1)
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestSnapshotIdValidation:
    """Tests for --snapshot-id option validation."""

    def test_zero_exits_nonzero(self, tmp_path: Path):
        """--snapshot-id 0 is rejected (must be positive)."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        result = runner.invoke(app, ["sbom", str(artifact), "--snapshot-id", "0"])
        assert result.exit_code != 0
        assert "positive integer" in result.output

    def test_negative_exits_nonzero(self, tmp_path: Path):
        """--snapshot-id -1 is rejected."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        result = runner.invoke(app, ["sbom", str(artifact), "--snapshot-id", "-1"])
        assert result.exit_code != 0
        assert "positive integer" in result.output

    def test_non_numeric_exits_nonzero(self, tmp_path: Path):
        """--snapshot-id abc is rejected."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        result = runner.invoke(app, ["sbom", str(artifact), "--snapshot-id", "abc"])
        assert result.exit_code != 0
        assert "positive integer" in result.output

    def test_float_exits_nonzero(self, tmp_path: Path):
        """--snapshot-id 3.5 is rejected."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        result = runner.invoke(app, ["sbom", str(artifact), "--snapshot-id", "3.5"])
        assert result.exit_code != 0
        assert "positive integer" in result.output

    def test_overflow_exits_nonzero(self, tmp_path: Path):
        """--snapshot-id with value > 2_147_483_647 is rejected."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        result = runner.invoke(app, ["sbom", str(artifact), "--snapshot-id", "2147483648"])
        assert result.exit_code != 0
        assert "positive integer" in result.output

    def test_empty_string_exits_nonzero(self, tmp_path: Path):
        """--snapshot-id '' (empty string) is rejected."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        result = runner.invoke(app, ["sbom", str(artifact), "--snapshot-id", ""])
        assert result.exit_code != 0
        assert "positive integer" in result.output

    def test_valid_positive_integer_accepted(self, tmp_path: Path):
        """--snapshot-id 42 passes validation and is forwarded to _run_sbom."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        ...
        with patch("debcraft.cli.sbom._run_sbom", new_callable=AsyncMock) as mock_run:
            ...
            result = runner.invoke(
                app,
                ["sbom", str(artifact), "--snapshot-id", "42", "--output-dir", str(tmp_path)],
            )
        assert result.exit_code == 0
        call_kwargs = mock_run.call_args.kwargs
        assert call_kwargs.get("snapshot_id") == 42

    def test_max_valid_value_accepted(self, tmp_path: Path):
        """--snapshot-id 2147483647 (max) passes validation."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        ...
        with patch("debcraft.cli.sbom._run_sbom", new_callable=AsyncMock) as mock_run:
            ...
            result = runner.invoke(
                app,
                ["sbom", str(artifact), "--snapshot-id", "2147483647", "--output-dir", str(tmp_path)],
            )
        assert result.exit_code == 0
        call_kwargs = mock_run.call_args.kwargs
        assert call_kwargs.get("snapshot_id") == 2_147_483_647

    def test_one_is_minimum_valid(self, tmp_path: Path):
        """--snapshot-id 1 (minimum valid) passes validation."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        ...
        with patch("debcraft.cli.sbom._run_sbom", new_callable=AsyncMock) as mock_run:
            ...
            result = runner.invoke(
                app,
                ["sbom", str(artifact), "--snapshot-id", "1", "--output-dir", str(tmp_path)],
            )
        assert result.exit_code == 0
        call_kwargs = mock_run.call_args.kwargs
        assert call_kwargs.get("snapshot_id") == 1

    def test_no_snapshot_id_passes_none(self, tmp_path: Path):
        """When --snapshot-id is not provided, None is passed to _run_sbom."""
        artifact = tmp_path / "artifact.deb"
        artifact.write_text("fake artifact")
        ...
        assert result.exit_code == 0
        call_kwargs = mock_run.call_args.kwargs
        assert call_kwargs.get("snapshot_id") is None
