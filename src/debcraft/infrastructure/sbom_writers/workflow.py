...
class SBOMWorkflow(Workflow):
    ...
    async def execute(self, context: WorkflowContext) -> None:
        ...
        try:
            ...
            writer_results, write_failures = await self._write_all_formats(context, writer_registry, sbom_document)
            ...
            # Raise on partial write failures (after persistence of successful formats)
            await self._check_write_failures(context, write_failures)

            # Publish completed event
            await self._log_completion(context, start_time, len(writer_results))
    ...
    async def _scan(
        ...
        """
        from debcraft.domain.scanner.values import Artifact, ArtifactType, detect_artifact_type
    ...
    async def _write_all_formats(
        ...
        """Execute the write step for all requested formats and collect results.

        Writes as many formats as possible, tracking both successes and
        failures.
        ...
        for fmt in self._config.formats:
            ...
            except Exception as exc:  # pylint: disable=broad-exception-caught  # Writer plugins may raise arbitrary errors; must not crash the workflow
    ...
    async def _check_write_failures(
        self,
        context: WorkflowContext,
        write_failures: list[tuple[OutputFormat, str]],
    ) -> None:
        """Check for partial write failures and raise if any occurred.

        Args:
            context: The workflow execution context.
            write_failures: List of (format, error_message) tuples.

        Raises:
            _WorkflowPartialFailure: If any formats failed to write.
        """
    ...
    async def _log_completion(
        ...
        """Publish workflow completed event and log success.

        Args:
            context: The workflow execution context.
            start_time: When the workflow started.
            formats_written: Number of formats successfully written.
        """
