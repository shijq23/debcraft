...
Rate-limit defaults:
- rate_limit_rps=50.0: 50 requests per second
- rate_limit_burst=20: resolved from max_connections_per_repo (burst=None means
  use max_connections_per_repo, which is 20 in the default config)
"""
...
class ConfigReader:
    ...
    def _build_config(self, data: dict) -> MirrorConfig:
        ...
        Raises:
            MirrorConfigurationError: If rate-limit fields are non-numeric or out of range.
        """
        ...
        rate_limit_rps = self._parse_rate_limit_rps(settings)
        rate_limit_burst = self._parse_rate_limit_burst(settings, max_connections_per_repo)
    ...
    def _parse_rate_limit_rps(self, settings: dict) -> float:
        ...
        Args:
            settings: The [settings] section from parsed TOML.

        Returns:
            Validated rate_limit_rps value.

        Raises:
            MirrorConfigurationError: If value is non-numeric or out of range [1, 1000].
        """
        raw_value = settings.get("rate_limit_rps", 50.0)

        if isinstance(raw_value, bool) or not isinstance(raw_value, (int, float)):
            raise MirrorConfigurationError(
                message=(
                    f"rate_limit_rps must be a numeric value (int or float), "
        ...
        if value < 1 or value > 1000:
            raise MirrorConfigurationError(
                message=(f"rate_limit_rps must be between 1 and 1000 (inclusive), got {value}"),
            )
    ...
    def _parse_rate_limit_burst(self, settings: dict, default_burst: int) -> int:
        ...
        Args:
            settings: The [settings] section from parsed TOML.
            default_burst: Default burst value (max_connections_per_repo).

        Returns:
            Validated rate_limit_burst value.

        Raises:
            MirrorConfigurationError: If value is non-numeric or out of range [1, 200].
        """
        raw_value = settings.get("rate_limit_burst")
        ...
        if not isinstance(raw_value, (int, float)):
            raise MirrorConfigurationError(
                message=(
                    f"rate_limit_burst must be a numeric value (int), got {type(raw_value).__name__}: '{raw_value}'"
                ),
            )
        ...
        if value < 1 or value > 200:
            raise MirrorConfigurationError(
                message=(f"rate_limit_burst must be between 1 and 200 (inclusive), got {value}"),
            )
