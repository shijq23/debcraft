...
class MirrorEngine:
    ...
    async def _stage_release(self, config: RepositoryConfig, suite: str) -> ReleaseMetadata | None:
        ...
        if inrelease_path.exists():
            is_unchanged = await self._download_coordinator.check_conditional(inrelease_url)
            if is_unchanged:
                self._logger.info(
        ...
        await self._upsert_repository_file(
            url=file_url,
            sha256="",  # Release file hash is self-referential
    ...
    async def _download_release_file(self, url: str, dest_path: Path) -> tuple[str, str] | None:
        ...
        Returns:
            Tuple of (file_content, url) on success, None on failure.
        """
